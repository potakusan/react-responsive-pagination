import { containsEllipsis, createEllipsis, createNavNext, createNavPrevious, createPage, isEllipsis, isNav, } from './compositionItem.js';
describe('isNav', () => {
...
export const shorthandOf = (received) => received?.map(({ type, page }) => {
    switch (type) {
        case 'page':
            return page;
        case 'active':
...
export const fromShorthand = (shorthand) => shorthand.map(shorthandItem => {
    if (shorthandItem === '<' ||
        shorthandItem === '>' ||
        shorthandItem === '…L' ||
        shorthandItem === '…R') {
